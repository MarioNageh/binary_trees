...
/**
 * r_insert_node - Recursively insert a new node into an AVL tree.
 * @tree: Pointer to the root node of the AVL tree struct.
 * @parent: Parent node of the current node being examined.
 * @new: Double pointer for insertion position.
...
avl_t *r_insert_node(avl_t **tree, avl_t *parent, avl_t **new, int nval)
{
	...
	if (balance_val > 1 && (*tree)->left->n > nval)
		*tree = binary_tree_rotate_right(*tree);
	else if (balance_val > 1 && (*tree)->left->n < nval)
	{
		(*tree)->left = binary_tree_rotate_left((*tree)->left);
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (balance_val < -1 && (*tree)->right->n < nval)
 ...
 * Return: Inserted node, or NULL if insertion fails.
 */
